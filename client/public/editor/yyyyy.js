...
class SvgPanZoom {
    ...
    constructor(Viewer) {
        ...
        new MutationObserver(function (mutations) {
            ...
            mutations.forEach(function (mutation) {
                ...
                document.getElementById('zoom').value = (SCALE * 100).toFixed(0) + '%';
    ...
    zoomOnWheel = () => {
        ...
        let doc = (document.head || document.body || document.documentElement)
...
class Viewer extends EventEmitter {
    ...
    constructor(id, option) {
        ...
        this.dom = document.getElementById("editor");
    ...
    create = {
        ...
        tools_left: () => {
            ...
            toolHandlerArray = [
                new EvenHandler(document.querySelectorAll('#tool_select')[0]).addEventListener('click', (evt) => this.emit('tool_select', evt)),
                new EvenHandler(document.querySelectorAll('#tool_fhpath')[0]).addEventListener('click', (evt) => this.emit('tool_fhpath', evt)),
                new EvenHandler(document.querySelectorAll('#tool_line')[0]).addEventListener('click', (evt) => this.emit('tool_line', evt)),
                new EvenHandler(document.querySelectorAll('#tool_rect')[0]).addEventListener('click', (evt) => this.emit('tool_rect', evt)),
                new EvenHandler(document.querySelectorAll('#tool_ellipse')[0]).addEventListener('click', (evt) => this.emit('tool_ellipse', evt)),
                new EvenHandler(document.querySelectorAll('#tool_path')[0]).addEventListener('click', (evt) => this.emit('tool_path', evt)),
                new EvenHandler(document.querySelectorAll('#tool_fill')[0]).addEventListener('click', (evt) => this.emit('tool_fill', evt)),
                new EvenHandler(document.querySelectorAll('#tool_stroke')[0]).addEventListener('click', (evt) => this.emit('tool_stroke', evt)),
                new EvenHandler(document.querySelectorAll('#tool_switch')[0]).addEventListener('click', (evt) => this.emit('tool_switch', evt))
            ];
            ...
            this.on('tool_fill', () => {
                ...
                var paint = new $.jGraduate.Paint({ alpha: 100, solidColor: $('#tool_fill svg rect').attr('fill') })

                // console.log($('#tool_fill svg rect'))

                $('#gradPicker').css({ 'left': 0, 'top': 0 }).jGraduate(
                    {
                        paint: paint,
