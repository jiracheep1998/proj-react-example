...
(function (global, factory) {
  ...
  var Helper = /*#__PURE__*/function () {
    function Helper(params) {
      ...
      if (typeof params === 'string') {
        var selector = base.document.querySelectorAll(params);
        this.length = selector.length;

        for (var count = 0; count < this.length; count++) {
          this[count] = selector[count];
        }
      } else if (_typeof(params) === 'object' && (params.nodeType === 1 || params === base.document)) {
        this[0] = params;
        this.length = 1;
    ...
    _createClass(Helper, [{
      ...
      value: function css(prop) {
        var _getStyle = function _getStyle(obj) {
          ...
          while (len--) {
            ...
            } else if (base.document.defaultView && base.document.defaultView.getComputedStyle) {
              return base.document.defaultView.getComputedStyle(obj[len], '')[prop];
            } else {
      ...
      value: function on() {
        ...
        while (len--) {
          ...
          if (typeof arguments[1] !== 'string') {
            if (base.document.addEventListener) {
              this[len].addEventListener(arguments[0], arguments[1], arguments[2] || {
                passive: false
              });
            } else if (base.document.attachEvent) {
      ...
      value: function off() {
        ...
        while (len--) {
          ...
          if (typeof arguments[1] !== 'string') {
            if (base.document.removeEventListener) {
              this[len].removeEventListener(arguments[0], arguments[1], arguments[2]);
            } else if (base.document.detachEvent) {
  ...
  var SubjectModel = /*#__PURE__*/function () {
    ...
    _createClass(SubjectModel, [{
      ...
      value: function _onMouseDown(e) {
        ...
        helper(base.document).on(E_MOUSEMOVE$1, this._onMouseMove).on(E_MOUSEUP$1, this._onMouseUp);
      }
    }, {
      key: "_onTouchStart",
      value: function _onTouchStart(e) {
        ...
        helper(base.document).on(E_TOUCHMOVE$1, this._onTouchMove).on(E_TOUCHEND$1, this._onTouchEnd);
      }
    }, {
      key: "_onMouseMove",
      ...
      value: function _onMouseUp(e) {
        ...
        helper(base.document).off(E_MOUSEMOVE$1, this._onMouseMove).off(E_MOUSEUP$1, this._onMouseUp);
      ...
      value: function _onTouchEnd(e) {
        helper(base.document).off(E_TOUCHMOVE$1, this._onTouchMove).off(E_TOUCHEND$1, this._onTouchEnd);
  ...
  var getScrollOffset = function getScrollOffset() {
    var doc = base.document.documentElement;
    return {
    ...
    _createClass(Transformable, [{
      ...
      value: function _processOptions() {
        ...
        this.options = {
          ...
          restrict: restrict ? helper(restrict)[0] || base.document.body : null,
      ...
      value: function disable() {
        ...
        if (storage.onExecution) {
          helper(base.document).off(E_MOUSEMOVE$2, this._onMouseMove).off(E_MOUSEUP$2, this._onMouseUp).off(E_TOUCHMOVE$2, this._onTouchMove).off(E_TOUCHEND$2, this._onTouchEnd);
        }

        elements.map(function (element) {
          removeClass(element, "".concat(LIB_CLASS_PREFIX, "drag"));
  ...
  var getCurrentTransformMatrix = function getCurrentTransformMatrix(element) {
    var container = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : base.document.body;
  ...
  var createElement = function createElement() {
    ...
    var element = base.document.createElement('div');
  ...
  var sepRE = /\s*,\s*|\s+/g;
  var allowedElements = ['circle', 'ellipse', 'image', 'line', 'path', 'polygon', 'polyline', 'rect', 'text', 'g', 'foreignobject', 'use', 'tspan'];
  function createSVGElement(name) {
    var classNames = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
    var element = base.document.createElementNS('http://www.w3.org/2000/svg', name);
    classNames.forEach(function (className) {
      return addClass(element, className);
    });
    return element;
  ...
  var DraggableSVG = /*#__PURE__*/function (_Transformable) {
    ...
    _createClass(DraggableSVG, [{
      ...
      value: function _init(elements) {
        ...
        // if(elements[0].href && elements[0].getAttribute('onclick')){
        //   let text = document.querySelector(elements[0].href.baseVal).querySelectorAll('tspan')[0];
        //   let image = document.querySelector(elements[0].href.baseVal).querySelectorAll('image')[0];
        //   if(text && text.innerHTML || image){
        //     resizingHandles = {};
        //   }
        // }
        ...
        let gWidth = createSVGElement('g');
        gWidth.setAttribute('id', 'g-width');

        let textWidth = createSVGElement('text');
        textWidth.setAttribute('id', 'text-width');
        textWidth.setAttribute('transform', 'translate(' + nextVertices.bc.x + ', ' + nextVertices.bc.y + ')')
        textWidth.style.fontSize = (12) + 'px';
        textWidth.setAttribute('fill', 'rgb(0, 168, 255)')

        gWidth.appendChild(textWidth);

        controls.appendChild(gWidth);

        let matrix = base.dom.querySelector('#svgroot').getCTM();//getMatrix(document.querySelectorAll('g#root')[0]);
        let viewScale = SCALE;

        let realSizeWidth = dimension.width / matrix.a / viewScale;
        let realSizeHeight = dimension.height / matrix.a / viewScale;

        textWidth.innerHTML = Math.round(realSizeWidth) + ' x ' + Math.round(realSizeHeight);

        setTimeout(() => {
          let size = textWidth.getBBox();
          let angle = (dimension.rotation > 0 ? dimension.rotation : (2 * Math.PI + dimension.rotation)) / 360 * (2 * Math.PI);
          let x = nextVertices.bc.x - ((size.width) / 2) * Math.cos(angle);
          let y = nextVertices.bc.y - ((size.width) / 2) * Math.sin(angle);
          let x1 = 0 - ((size.width) / 2.3) * Math.sin(angle);
          let y1 = 0 + ((size.width) / 2.3) * Math.cos(angle);
          base.document.getElementById('g-width').setAttribute('transform', 'translate(' + (x1) + ', ' + (y1) + ')');
          textWidth.setAttribute('transform', 'translate(' + (x) + ', ' + (y) + ') rotate(' + dimension.rotation + ')');
        });

        let gAngle = createSVGElement('g');
        gAngle.setAttribute('id', 'g-angle');

        let textAngle = createSVGElement('text');
        textAngle.setAttribute('id', 'text-angle');
        textAngle.setAttribute('transform', 'translate(' + nextVertices.tc.x + ', ' + nextVertices.tc.y + ')')
        textAngle.style.fontSize = (12) + 'px';
        textAngle.setAttribute('fill', 'rgb(0, 168, 255)')

        gAngle.appendChild(textAngle);

        controls.appendChild(gAngle);

        textAngle.innerHTML = Math.floor(dimension.rotation) + '°';

        setTimeout(() => {
          let sizeAngle = textAngle.getBBox();
          let angle = (dimension.rotation > 0 ? dimension.rotation : (2 * Math.PI + dimension.rotation)) / 360 * (2 * Math.PI);
          let x = nextVertices.tc.x - ((sizeAngle.width) / 2) * Math.cos(angle);
          let y = nextVertices.tc.y - ((sizeAngle.width) / 2) * Math.sin(angle);
          let x1 = 0 + ((sizeAngle.width) / 1.5) * Math.sin(angle);
          let y1 = 0 - ((sizeAngle.width) / 1.5) * Math.cos(angle);
          base.document.getElementById('g-angle').setAttribute('transform', 'translate(' + (x1) + ', ' + (y1) + ')');
          textAngle.setAttribute('transform', 'translate(' + (x) + ', ' + (y) + ') rotate(' + dimension.rotation + ')');
        });
      ...
      value: function _processResize(element, _ref4) {
        ...
        var getScale = function getScale(distX, distY) {
          ...
          if (!doW && !doH) {
            newWidth = actualBoxWidth * ratio;
            newHeight = actualBoxHeight * ratio;
          } else {
            newWidth = actualBoxWidth + distX;
            newHeight = actualBoxHeight + distY;
          }
      ...
      value: function _getPoints() {
        ...
        var nextTransform = isGrouped ? transformMatrix : transformMatrix.multiply(getTransformToElement(elements[0], elements[0].parentNode));
        var nextVertices = entries$1(vertices).reduce(function (nextRes, _ref9) {
          var _ref10 = _slicedToArray(_ref9, 2),
            key = _ref10[0],
            _ref10$ = _slicedToArray(_ref10[1], 2),
      ...
      value: function _processControlsResize() {
        ...
        let textWidth, textAngle;

        if (textWidth = base.document.getElementById('text-width')) {

          let matrix = base.document.getElementById('svgroot').getScreenCTM();
          let viewScale = panZoom.getSizes().realZoom;
        ...
        if (textAngle = base.document.getElementById('text-angle')) {
          textAngle.innerHTML = Math.floor(dimension.rotation) + '°';
          let sizeAngle = textAngle.getBBox();
          let angle = (dimension.rotation > 0 ? dimension.rotation : (2 * Math.PI + dimension.rotation)) / 360 * (2 * Math.PI);
          let x = nextVertices.tc.x - ((sizeAngle.width) / 2) * Math.cos(angle);
      ...
      value: function _updateControlsView() {
        ...
        let normal, te, be, le, re, rotate, tl, tr, br, bl, tc, bc, ml, mr, textWidth, textAngle;

        if (normal = base.document.getElementsByClassName('sjx-svg-line-normal')[0]) {
          normal.setAttribute('x1', anchor.x);
          normal.setAttribute('y1', anchor.y);
          normal.setAttribute('x2', rotator.x);
          normal.setAttribute('y2', rotator.y);
          normal.setAttribute('stroke-width', 1);
        }

        if (te = base.document.getElementsByClassName('sjx-svg-line-te')[0]) {
          te.setAttribute('x1', resizingEdges.te[0].x);
          te.setAttribute('y1', resizingEdges.te[0].y);
          te.setAttribute('x2', resizingEdges.te[1].x);
          te.setAttribute('y2', resizingEdges.te[1].y);
          te.setAttribute('stroke-width', 1);
        }

        if (be = base.document.getElementsByClassName('sjx-svg-line-be')[0]) {
          be.setAttribute('x1', resizingEdges.be[0].x);
          be.setAttribute('y1', resizingEdges.be[0].y);
          be.setAttribute('x2', resizingEdges.be[1].x);
          be.setAttribute('y2', resizingEdges.be[1].y);
          be.setAttribute('stroke-width', 1);
        }

        if (le = base.document.getElementsByClassName('sjx-svg-line-le')[0]) {
          le.setAttribute('x1', resizingEdges.le[0].x);
          le.setAttribute('y1', resizingEdges.le[0].y);
          le.setAttribute('x2', resizingEdges.le[1].x);
          le.setAttribute('y2', resizingEdges.le[1].y);
          le.setAttribute('stroke-width', 1);
        }

        if (re = base.document.getElementsByClassName('sjx-svg-line-re')[0]) {
          re.setAttribute('x1', resizingEdges.re[0].x);
          re.setAttribute('y1', resizingEdges.re[0].y);
          re.setAttribute('x2', resizingEdges.re[1].x);
          re.setAttribute('y2', resizingEdges.re[1].y);
          re.setAttribute('stroke-width', 1);
        }

        if (rotate = base.document.getElementsByClassName('sjx-svg-hdl-rotator')[0]) {
          rotate.setAttribute('cx', rotator.x);
          rotate.setAttribute('cy', rotator.y);
          rotate.setAttribute('r', 4);
          rotate.setAttribute('stroke-width', 1);
        }

        if (tl = base.document.getElementsByClassName('sjx-svg-hdl-tl')[0]) {
          tl.setAttribute('cx', resizingHandles.tl.x);
          tl.setAttribute('cy', resizingHandles.tl.y);
          tl.setAttribute('r', 4);
          tl.setAttribute('stroke-width', 1);
        }

        if (tr = base.document.getElementsByClassName('sjx-svg-hdl-tr')[0]) {
          tr.setAttribute('cx', resizingHandles.tr.x);
          tr.setAttribute('cy', resizingHandles.tr.y);
          tr.setAttribute('r', 4);
          tr.setAttribute('stroke-width', 1);
        }

        if (br = base.document.getElementsByClassName('sjx-svg-hdl-br')[0]) {
          br.setAttribute('cx', resizingHandles.br.x);
          br.setAttribute('cy', resizingHandles.br.y);
          br.setAttribute('r', 4);
          br.setAttribute('stroke-width', 1);
        }

        if (bl = base.document.getElementsByClassName('sjx-svg-hdl-bl')[0]) {
          bl.setAttribute('cx', resizingHandles.bl.x);
          bl.setAttribute('cy', resizingHandles.bl.y);
          bl.setAttribute('r', 4);
          bl.setAttribute('stroke-width', 1);
        }

        if (tc = base.document.getElementsByClassName('sjx-svg-hdl-tc')[0]) {
          tc.setAttribute('cx', resizingHandles.tc.x);
          tc.setAttribute('cy', resizingHandles.tc.y);
          tc.setAttribute('r', 4);
          tc.setAttribute('stroke-width', 1);
        }

        if (bc = base.document.getElementsByClassName('sjx-svg-hdl-bc')[0]) {
          bc.setAttribute('cx', resizingHandles.bc.x);
          bc.setAttribute('cy', resizingHandles.bc.y);
          bc.setAttribute('r', 4);
          bc.setAttribute('stroke-width', 1);
        }

        if (ml = base.document.getElementsByClassName('sjx-svg-hdl-ml')[0]) {
          ml.setAttribute('cx', resizingHandles.ml.x);
          ml.setAttribute('cy', resizingHandles.ml.y);
          ml.setAttribute('r', 4);
          ml.setAttribute('stroke-width', 1);
        }

        if (mr = base.document.getElementsByClassName('sjx-svg-hdl-mr')[0]) {
          mr.setAttribute('cx', resizingHandles.mr.x);
          mr.setAttribute('cy', resizingHandles.mr.y);
          mr.setAttribute('r', 4);
      ...
      value: function _applyTransformToHandles() {
        ...
        let pathList = this.elements[0].querySelectorAll('path');

        for (let i = 0; i < pathList.length; i++) {
          let matrix = pathList[i].getCTM();
          let a = matrix.a;
          let stroke = 1 / a;

          pathList[i].setAttribute('stroke-width', stroke);
        }
  ...
  var Cloneable = /*#__PURE__*/function (_SubjectModel) {
    ...
    _createClass(Cloneable, [{
      ...
      value: function _init() {
        ...
        elements.map(function (element) {
          return data.set(element, {
            parent: isDef(appendTo) ? helper(appendTo)[0] : base.document.body
          });
        });
        this.storage = {
          css: css,
      ...
      value: function _processOptions(options) {
        ...
        if (isDef(options)) {
          ...
          var dropZone = isDef(stack) ? helper(stack)[0] : base.document;
